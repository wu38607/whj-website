...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$http.post('/api/admin/blog/update', this.blog).then((res) => {
        res = res.body;
        if (res.errno === this.ERR_OK) {
          ...
          alert('上传成功！');
          eventBus.$emit('onBlogUpdate', data);
  ...
  mounted() {
    ...
    this.$http.get(`/api/public/blog/${id}.json`).then((res) => {
      res = res.body;
      if (res.errno === this.ERR_OK) {
        let blog = res.data.blog;
        this.blog.title = blog.title;
        this.blog.md = blog.md;
        this.blog.guide = blog.guide;
        this.blog.pv = blog.pv;
      } else {
        alert('博客加载失败');
      }
    });
